// wrote_240930.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//

#include <iostream>

//값을 명시하는 순간 구현이 된다.
//실체(구현) = 메모리화
class monster
{
public:
	int a = 0; //구현인가? 아니다! 선언이다.
};

monster MyMonster; //이때 itn a 가 구현된다.

//클래스는 기본적으로 선언이다.
//클래스는 동시에 선언 불가능하다. (같은 이름)
class Player {};
class Player {};  //이런게 불가능하다.

//전역 함수의 선언... 한 파일 내에서 중복 가능? 그렇다.

// 
// 자료형의 구분
// 1. 기본자료형... 재정의 불가능 
// 2. 사용자 정의 자료형

int main()
{
	monster New; //구현
	int a; //선언
}
//선언만 하고 사용을 안 하면 컴파일러가 의미 없다고 생각한다.
//자료형 안에 어떤 기능이 있다면 그걸 구현해줘야 한다.
// 
//한 파일 (=하나의 cpp) 내에서 선언 중복이 가능한 문법이 있다.
//함수... 전역함수와 멤버함수(클래스)
//자료형, 객체, 전역함수의 선언과 구현에 대한 이해
//

class Battle
{
public:
	void Test() //멤버함수의 구현이지 클래스의 구현이 아니다.
	{

	}
};

//하나의 파일 안에서 같은 이름은 선언 불가능하다.

//확장자 의미 없어...h와 cpp라도 다 텍스트 파일로 인식한다.
//
//구현은 중복 불가능하다.

//언리얼 스탠다드.... 클래스명 앞에 U를 붙인다.
// class UFightunit {};

//****코더와 아키텍처의 차이? 프로젝트 설정을 할 줄 알아야 한다.****

//헤어 안에는 최대한 헤더가 없을수록 좋다. 순환 참조 때문에.
//#include ""  내가 만든 파일,
//#include <>  외부 파일 (VC++ 디렉터리의 포함 디렉터리를 확인해보자 )
// 
// 라이브러리...dll, lib 등의 위치. 
// (진짜 실행 파일은 아니지만 다른 프로젝트에서 사용하기 위해 만든 코드들의 위치)
//디렉터리들 안 만질거다. 에러난다.

//다시 돌아와서... 헤더 순환참조가 뭐지?
class Parent
{
public:
	Child Child0;  //(부모가 자식을 알면 안 된다.)
};

class Child: public Parent
{

};
//이것이 순환참조.
//서클형이 가장 위험하다.
//#include를 최소화하자.
// 
//(그러나 상속관계 만큼은 #include를 피할 수 없다.
// 자식의 크기를 알려면 부모의 크기를 알아야 한다.)
// 포인터는 상관없다. 무조건 크기가 8바이트니까.
// 
// 헤더(.h)에서 함수를 구현하지 않음.
// 어떤 클래스의 함수를 사용하려면 호출을 위해 헤더가 필요하기 때문에.
// 잘못하면 순환구조가 만들어진다.
// 반드시 헤더는 선언만!
// cpp에 #include
// 헤더는 코드를 짜고 마지막에 추가하는 습관을 들이자. 

//순환 방지를 위해...
//상위개념(좀 더 근본적인)은 항상 위에. ('중력'이 '플레이어'를 알아야 하나? 아니다.)
// 
//


//전방선언?
class Weapon Axe;
// 마치 extern 처럼 있을거라고 선언해주는 것.
// 전방선언은 중복 가능하다.
// 수업에선 적당히 사용
// 
// 

//객체지향...최대한 전역변수를 안 쓰도록 하자. (전역적인 요소를 안 쓰는 것.)

//const는 선언과 구현을 분리하지 않아도 됨.

//base object contents
//base 수학과 같은 연산들, 제일 순수한 코드들
//object 
//contents 실제 메모리화 되는 코드들 (플레이어 몬스터 맵 등등)
//

// endl은 \n와 같은 일을 하는 객체
// 

//파일 입출력
//(헤더 달아야 한다!)
//클래스 통째로 저장한다.
// 
// struct
// 클래스와 문법이 완전히 동일하다.
// 딱 하나 다르다. 기본 접근제한지정자가 public이다. 그 외에 진짜 다 똑같다.
// 언제 쓸까? 사람마다 다르겠지 뭐.
// 웬만한 변수는 public으로 하고 싶을때.
// 
// void*가 뭐지?
// 크기와 상관없이 주소값만 사용하게하는 포인터. 순수하게 위치의 의미만 가지게 된다.
int* IPtr = nullptr;
char* CPtr = nullptr;

void* VPtr = IPtr;   
void* VPtr = CPtr;  //에러 안 난다.
// 원칙적으로 pointer는 함부로 못 바꿈, 암시적 형변환 불가. (연산 때문에)
// 포인터끼리 호환이 안 된다. (short*를 int*로 바로 바꿀 수 없다. 형변환 필요.)
// 
// fopen
// (참고로 _s는 security. 메모리를 확인하기 힘든 함수를 의미한다.)
// 핸들방식, 하드디스크의 파일에 간섭할 수 있는 주소를 줌.
// 모드 (w = 쓴다, r= 읽는다)(b = 이진수, t = 텍스트)
// t에서 주의할 점, 읽다가 엔터를 만나면 종료
// 
//
//fopen_s(&File, )
//파일이 없다면 만들어낸다.
fwrite("11111111111", 12, 1, File); //12바이트를 한번
//텍스트로 저장되어도 메모장에서 열면 유니코드가 적용되서 한글이 잘 나온다.
// istream? ostream? C스타일 
// 랩핑...
//

// ***세상에 놀라운 일이?! char에 대한 포인터가 없다?! 뿌슝빠슝(오버로딩 안 되어 있는 듯)***

