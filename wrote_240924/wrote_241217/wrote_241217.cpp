...
// 
// Device가 만들어지지 않으면 리소스 로드도 할 수 없다.
// 윈도우 크기가 정해져야 그것을 기반으로 리소스를 로드한다.
// 
// CreateDeviceAndContext()와 CreateBackbuffer()
// 
...
// 
// pAdapter
// 그래픽 사양 정보를 아려준다. nullptr를 넣어주면 알아서 찾는다.
// 만약 그래픽 카드가 여러개라면...비교적 고사양의 경우를 확인하는 함수를 만들면 된다.
// GetHigherPerformanceAdapter()
// 
// Factory 다렉에서 지원하는, 그래픽 카드 메모리 생성을 담당하는 인터페이스
// 
// 다렉의 라이브러리들은 클래스를 인지하는게 아니라
// GUID라는 것으로 코드들을 그때그때 로드하는 방식
// 
