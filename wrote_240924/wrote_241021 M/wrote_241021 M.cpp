...
// 
//               의미:
 #define Apple  //Apple은 존재한다.

 #ifdef Apple   //Apple이 존재한다면 전처리기가 이 안의 코드를 넣는다.
 #else          //Apple이 없다면 전처리기가 이 안의 코드를 넣는다.
                //(Apple이 존재한다면 이 문장은 흐리게 처리된다.)
 #endif

// (멀티바이트, 유니코드에 따라서 함수명이 달라지는게 귀찮다면
//  define을 이용해서 수정할 필요없게 만들 수 있다.
//  그러나 두번 쳐야하니까 비추)
 
// "" <- 멀티바이트, 최신 라이브러리는 char*를 요구한다. 
// utf계열 인코딩은 char로 표현되기 때문이다. (utf는 1~4바이트로 글자 표현)
// wchar_t는 2바이트 즉, 1바이트 단위는 표현할 수 없다.
// (선생님은 무조건 멀티바이트로)
//
...
// 
    int Value = 1;
             // 00000000 00000000 000000000 000000001  (4바이트)
    char Arr[4];
    int* Ptr = reinterpret_cast<int*>(Arr);
// Arr 배열 원소중 누가 1일까?
// 
// CPU 회사에 따라 바이트 순서를 다르게 넣는다.
// Little Endian 순서를 뒤집어서 (Intel)  Arr[0] = 1
// Big Endian  바이트 그대로 (AMD)  Arr[3] = 1
// 
// 글로벌 서비스를 하는 경우가 아니면 별 문제 없다.
//
...
// 
// 기본 메시지 루프...
// 윈도우 창에 일이 발생하거나 프로그램이 실행중인 경우
// 중단점이 걸리지 않는다면 프로그램은 아무것도 하지 않는다는 것.
// 윈도우 창에 있어서 일이란? 
// 윈도우 선택, 윈도우 이동, 윈도우 클릭, 최대화, 최소화 등...
// 이런 일들이 일어나면 메시지가 발생
// 
// 일이 일어나면 뭘 해야하지? 
...
// {}
// 윈도우 창에 무슨 일이 생긴다면 WndProc 함수를 실행시킨다.
// HWND hwnd 어떤 윈도우창인지
// UINT message
// 
// 
// 마치 enum처럼 작동한다.
//
// WM_CREATE     윈도우 창이 처음 생길때 호출
// WM_KILLFOCUS  윈도우 창이 선택되지 않았다
// WM_SETFOCUS   윈도우 창이 선택됐다         등등...
// 
// 근데 게임은 선택되지 않아도 돌아가는 경우가 대부분이지.
// 즉, 메시지가 없어도 프로그램은 실행된다. 
//
