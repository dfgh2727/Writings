...
// 
// 사실 C++에서 블루프린트 함수를 호출하면 안 된다.
// 구조를 살펴보면 엔진과 컨텐츠보다 하위에 블루 프린트가 있기 때문이다.
// 
// Event 이벤트
// :일련의 노드들을 하나로 묶을 수 있다.
// 
// 코드로 다음과 같이 함수를 만들고 옵션을 지정하면
// 블루 프린트에서 구현할 수 있는 이벤트
// UFunction(BluePrintImplementableEvent)
// 언리얼이 구현해버리기 때문에 직접 구현하면 2번 구현되므로 에러가 발생한다.
// BluePrintImplementableEvent(); (X)
// 
// 결론: CPP에서 블루프린트로, 블루프린트에서 CPP로 호출할 수 있다. 쌍방 가능하다.
