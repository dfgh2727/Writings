...
// Window에서 제공하는 IO관리 멀티쓰레드 모델 
// 초 대용량의 파일이나 초 다수의 유저들과 서버 통신을 할 때 이용한다.
// I/O (Input Output)
// 프로그램으로 외부 데이터가 들어오거나, 프로그램 밖으로 데이터가 나가는 것
// 내 프로세스의 램에 존재하는 데이터가 아닌 모든 외부 데이터를 입출력 받는것을 IO라 한다.
// 윈도우가 가장 모범적이고 자신들의 OS에 가장 최적화된 모델을 제시했는데 그 모델이 IOCP.
// 
// IO기능을 제외하고 쓰레드 관리 부분만 사용할 것이다.
// (게임 프로그래머는 대용량 파일을 읽는 것 등등...보다 쓰레드 관리에 이용한다.)
// 
// 스레드는 복사생성자가 막혀있다.
// 
// CreateIoComletionPort(핸들,(파일을 관리하고 싶으면 FILE),  )
// 윈도우에게 IOCP를 이용해서 쓰레드를 관리하겠음을 알림.
// 혹은 서버통신을 할 시에 파일을 관리할 수도 있다.
// 
...
// 쓰레드 갯수만큼 만들어주고 켜주자.
// 
// ThreadQueueFunction(_JobQueue->)
// {
//		while(true)  쓰레드를 살려놓기 위해 while 사용
//		{
//			GetQueuedCompletionStatus(핸들, ..., 1000)
//			핸들이 주체로 작용, 스레드는 일이 있을때까지 잔다...
//			1000mS->아무 일이 없어도 1초 마다 일어난다.
//		}
// }
// 
// PostQueueCompletionStatus();
// 쓰레드풀을 만들때 가장 핵심인 함수,
// IOCP 쓰레드 중 가장 적절한 스레드를 깨운다.
// (8바이트 정수를 전달할 수 있다면...모든 것을 전달 할 수 있어!)
// 함수 포인터를 담아서 날린다면 쓰레드에서 실행한다...
// 
// 어떻게 다시 재울까?
// enum class를 만들고...
// RunningCount가 0이 될 때까지 계속 끈다.
